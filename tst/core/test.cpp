...
struct AudioMetadata{
  ...
  void print(){
    ...
    std::cout << "  Audio Format Code: " << audioFormatCode << '\n';
    ...
    std::cout << "  Little-Endian?: " << littleEndian << "\n\n";

    std::cout << "Time & Frequency:\n";
    std::cout << "  Duration: " << durationSeconds << " sec\n";
    std::cout << "  Average Frequency: " << avgFrequency << " Hz\n";

    std::cout << "Amplitude Analysis:\n";
    std::cout << "  Min Amplitude: " << minAmplitude << '\n';
    std::cout << "  Max Amplitude: " << maxAmplitude << '\n';
    std::cout << "  RMS Amplitude: " << rmsAmplitude << '\n';
    std::cout << "  Clipping Detected?: " << clippingDetected << "\n\n";

    std::cout << "Compression / byteRate Info:\n";
    std::cout << "  Is Variable Bit Rate (VBR)?: " << isVBR << '\n';
    std::cout << "  Bit Rate: " << bitrateKbps << " kbps\n\n";
...
private:
  ...
  bool loadWAV(const std::string& path,AudioMetadata& meta){
    ...
    switch (hdr.bitsPerSample){
      ...
      case 32:
               if (hdr.audioFormat==3) // IEEE float
                 meta.sampleType=AudioMetadata::SampleType::FLOAT32;
               else
                 meta.sampleType=AudioMetadata::SampleType::INT32;
               break;
      default:
               std::cerr << "Unsupported bit depth: " << hdr.bitsPerSample << "\n";
               return false;
    }
