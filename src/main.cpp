...
void drawButton(WINDOW *menuwin, int y, int x, const std::string &text, bool highlighted) {
    ...
    // Top border
    mvwprintw(menuwin, y, x, "%s", tl);
    for (int j = 0; j < btn_width; j++) mvwprintw(menuwin, y, x + 1 + j, "%s", h);
    mvwprintw(menuwin, y, x + btn_width + 1, "%s", tr);

    // Middle row
    mvwprintw(menuwin, y + 1, x, "%s", v);
    ...
    mvwprintw(menuwin, y + 1, x + btn_width + 1, "%s", v);

    // Bottom border
    mvwprintw(menuwin, y + 2, x, "%s", bl);
    for (int j = 0; j < btn_width; j++) mvwprintw(menuwin, y + 2, x + 1 + j, "%s", h);
    mvwprintw(menuwin, y + 2, x + btn_width + 1, "%s", br);
}
...
int main() {
    ...
    clear();
    mvprintw(height / 2, (width - 20) / 2, "You selected: %s", options[choice].c_str());
    refresh();
